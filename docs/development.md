...
`0666ad2b2b2452668733729e8b54234f5964643a` during the Docker build.
...
docker build \
  ...
  --target server \
  -t inference-server-llama-rocm:7.2.1-0666ad2 \
  .
```
...
docker build \
  ...
  --target server \
  -t inference-server-llama-vulkan:26.04-0666ad2 \
  .
```
